...
function createDropdownBtn(label, items, columns){
    let grid = createDropdownGridContent(items.map(item => `<div class="chkbx"><label class="chkLabel">&#9900; ${item.name}</label><input class="dropdown-item" type="checkbox" id="inputChkbx" data-key="${item.name}" name="${item.name}"/></div>`), columns)
